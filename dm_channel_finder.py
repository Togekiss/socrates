...
def cull_json(json_data, categories_to_keep):
...
def main():
    ...
    cli_command = "dotnet DCE/DiscordChatExporter.Cli.dll channels -g 857848490683269161 -t MTEyOTQzMjk5MjYwODE2NTk5MA.GthCAW.-rxzuOWr7JvzFI6f1XyVwfsRu7tXVVMUTS4ITA"
    ...
    categories_txt_file = "res/dm_categories_keep.txt"
    categories_to_keep = read_categories_from_txt(categories_txt_file)

    # Cull the JSON data by removing the entries with matching categories
    culled_json_data = cull_json(parsed_data, categories_to_keep)
